            ...
            <div class="col-md-12 stretch-card">
                <div class="card">
                    <div class="card-body">
                        ...
                        <a class="card-title badge badge-info rounded" href="{{ route('loadDiscount_code') }}">Thêm
                            mới</a><br>
                                ...
                                <tbody>
                                    @foreach ($allDisc as $disc)
                                        <tr>
                                            <td>{{ $disc->id }}</td>
                                            <td>{{ $disc->code }}</td>
                                            <td>{{ $disc->discount }} VNĐ</td>
                                            <td>{{ $disc->quantity }}</td>
                                            <td>{{ $disc->start_time }}</td>
                                            <td>{{ $disc->end_time }}</td>
                                            <td><a class="badge badge-info rounded"
                                                    href="{{ route('loadUpdateDiscount_code', $disc->id) }}"><i
                                                        class="mdi mdi-wrench"></i></a>
                                                <a class="badge badge-danger rounded"
                                                    onclick="return confirm('Xóa mục này?')"
                                                    href="{{ route('deleteDiscount_code', $disc->id) }}"><i
