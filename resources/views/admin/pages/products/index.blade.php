                            ...
                            <form class="card-title" action="{{ route('search7') }}" method="GET">
                                ...
                                <div class="form-group">
                                    <select class="form-control show-cti form-select list" name="keywords_price"
                                        id="price" onchange="this.form.submit()">
                            ...
                            <table id="recent-purchases-listing" class="table table-hover">
                                ...
                                <tbody>
                                    @foreach ($allPro as $pro)
                                        <tr>
                                            <td>{{ $pro->id }}</td>
                                            <td> {{ $pro->Cate_item->name }}</td>
                                            <td>{{ $pro->name }}</td>
                                            <td><img src="{{ asset('images/products/' . $pro->image) }}" alt=""
                                                    style="width:90px; height: auto !important;"></td>
                                            ...
                                            <td>
                                                <a class="badge badge-info rounded"
                                                    href="{{ route('showVariants', $pro->id) }}">Biến thể</a>
                                                <a class="badge badge-info rounded"
                                                    href="{{ route('loadEditPro', $pro->id) }}"><i
                                                        class="mdi mdi-wrench"></i></a>
                                                <a class="badge badge-danger rounded"
                                                    onclick="return confirm('Xóa mục này?')"
                                                    href="{{ route('deletePro', $pro->id) }}"><i
